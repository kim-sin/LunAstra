...
Use `LunAstra-3.2.0-source.zip`. Extract it and put the contents of its `LunAstra` directory at the repository root, including `.github`, `.gitignore` and `.gitattributes`. Do not upload only a ZIP as the repository source.
...
python3 tools/release.py --source-output ../LunAstra-3.2.0-source.zip
python3 tools/release.py --output ../LunAstra-3.2.0.zip
python3 tools/release.py --verify ../LunAstra-3.2.0.zip
```
...
The repository includes no automatic public-release publishing action. Posting a release remains the repository owner's explicit action. The normal CI workflow needs read-only repository permissions; inspect the first Actions result before promoting a release.
