...
2. In your active Codex host, open `/hooks`, review the LunAstra definitions and approve them. Start a new Luna conversation. Review native child-model/reasoning defaults before dispatch; full-history forks alone do not override those defaults. Confirm the effective child model/effort in the host before a light-Luna benchmark. Do not restart existing research or workers for this check.
